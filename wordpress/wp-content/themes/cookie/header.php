...
<?php function agni_header(){ 
	...
	<?php 
	$has_padding = ''; if( $cookie_options['layout-padding'] == '1' ){ $has_padding = ' has-padding'; }
	$has_sidebar = ''; if( $cookie_options['header-menu-style'] == 'side-header-menu' ){ $has_sidebar = ' has-sidebar'; }

	$has_animation_mobile = ''; if( $cookie_options['animation-mobile'] == '1' ){ $has_animation_mobile = ' has-animation-mobile'; }
	$has_parallax_mobile = $skrollr_body = ''; if( $cookie_options['parallax-mobile'] == '1' ){ $has_parallax_mobile = ' has-parallax-mobile'; $skrollr_body = 'id="skrollr-body"'; }

	 ?>
	<body <?php echo $skrollr_body; ?> <?php body_class( $has_padding.$has_sidebar.$has_animation_mobile.$has_parallax_mobile ); ?>>
	...
	else if( esc_attr( get_post_meta( $page_id, 'page_transparent', true ) ) == 'on' ){
	...
	/* Header Menu */
	ob_start();
		wp_nav_menu( array( 'menu_class' => 'nav-menu-content', 'menu_id' => 'navigation', 'container' => false, 'theme_location' => 'primary', 'fallback_cb'     => 'wp_page_menu' ) ); 
		$header_menu = ob_get_contents();
	ob_end_clean(); ?>
	<?php 
	/* Header Icon */
	ob_start(); ?>	
		<div class="header-icon <?php if( !empty($cookie_options['logo-2']['url']) ){ echo 'header-icon-additional-logo '; }?><?php if( !empty($cookie_options['header-logo-bg-color-2']) ){ echo 'header-logo-additional-bg-color'; }?>">
			 <?php if(!empty($cookie_options['logo-1']['url']) && $cookie_options['header-site-title'] == '0'){  ?>
				<a href="<?php echo esc_url( home_url( '/' ) ); ?>" class="logo-icon <?php echo esc_attr($logo_1_class); ?>"><img src="<?php echo esc_html($cookie_options['logo-1']['url']); ?>" alt="<?php echo esc_attr( get_bloginfo( 'name' ) ); ?>"></a><?php 
			} 
			if (!empty($cookie_options['logo-2']['url']) && $cookie_options['header-site-title'] == '0' ) { ?>
				<a href="<?php echo esc_url( home_url( '/' ) ); ?>" class="logo-icon <?php echo esc_attr($logo_2_class); ?>"><img src="<?php echo esc_html($cookie_options['logo-2']['url']); ?>" alt="<?php echo esc_attr( get_bloginfo( 'name' ) ); ?>"></a><?php 
			} 
			if ($cookie_options['header-site-title'] == '1' ) { ?>
	...
	ob_start(); ?>	
		...
		<?php } ?>
				<ul class="social-icons list-inline">
					<?php if( $cookie_options['social-media-header'] == '1' ){ 
						foreach( $cookie_options['social-media-icons-header'] as $social_checkbox => $social_icons ){
							if( $social_icons == '1' ){ ?>
	...
	ob_start(); ?>	
		<div class="header-menu-icons <?php if( !empty($cookie_options['header-icon-link-color-2']) ){ echo 'header-menu-icons-additional-color'; } ?>">
			...
			<?php if( $cookie_options['header-cart-box'] == '1' ){ 
				if(is_plugin_active( 'woocommerce/woocommerce.php')){?> 
					<div class="header-cart-toggle header-toggle">
						...
						<?php if($cookie_options['header-menu-style'] != 'side-header-menu'){ the_widget( 'WC_Widget_Cart' ); } ?>
					</div>
				<?php } 
			} ?>
		</div>
	...
	<div id="page" class="hfeed site wrapper <?php if( $cookie_options['layout-boxed'] == '1' ){ echo 'boxed'; } ?> ">
		<header id="masthead" class="site-header" role="banner">            
			<!-- Header -->       
			<?php if( $cookie_options['header-top-bar'] == '1' && $cookie_options['header-menu-style'] != 'side-header-menu' ){ ?>
				<div class="header-top-bar <?php if( $cookie_options['fullwidth-header-menu'] == '1'){ echo 'fullwidth-header-menu '; } ?><?php if( $header_transparent == '1' ){ echo 'transparent-nav-menu transparent-native '; } ?><?php if( $cookie_options['header-sticky'] == '1' ){ echo 'top-sticky '; } ?>">
					<div class="container<?php if( $cookie_options['fullwidth-header-menu'] == '1'){ echo '-fluid'; } ?>">
						<?php if( !empty($cookie_options['header-top-email']) ){ ?><span class="top-bar-email"><i class="fa fa-envelope-o"></i><?php echo esc_html($cookie_options['header-top-email']); ?></span><?php } ?>
						<?php if( !empty($cookie_options['header-top-number']) ){ ?><span class="top-bar-number"><i class="fa fa-mobile-phone"></i><?php echo esc_attr($cookie_options['header-top-number']); ?></span><?php } ?>
						<?php if( $cookie_options['social-media-header'] == '1' && $cookie_options['social-media-header-location'] == '2' ){ echo $header_social; }?>
						<?php if( $cookie_options['top-bar-nav'] == '1' ){ ?>
			...
			<?php } ?>
		<div class="header-navigation-menu <?php if( $cookie_options['fullwidth-header-menu'] == '1'){ echo 'fullwidth-header-menu '; } ?><?php if( $header_transparent == '1' ){ echo 'transparent-nav-menu transparent-native '; } ?><?php if( $cookie_options['header-menu-style'] == 'side-header-menu' ){ echo 'side-header-menu ';} ?><?php if( $cookie_options['header-menu-style'] == 'minimal' ){ echo 'minimal-nav-menu ';} ?><?php if( $cookie_options['shrink-header-menu'] == '1' ){ echo 'shrink-header-menu shrink-native '; } ?><?php if( $cookie_options['header-menu-style'] == 'centered-menu' || $cookie_options['header-menu-style'] == 'centered-logo' ){ echo 'center-header '; }?><?php if( $cookie_options['header-sticky'] == '1' ){ echo 'header-sticky '; } ?><?php if( !empty($cookie_options['header-menu-bg-color-2']) ){ echo 'header-additional-bg-color '; }?><?php if( !empty($cookie_options['header-menu-border-2']) ){ echo 'header-menu-border-additional '; }?><?php echo esc_attr($reverse_skin); ?> clearfix">
			<?php if( $cookie_options['header-menu-style'] == 'centered-logo' || $cookie_options['header-menu-style'] == 'side-header-menu' ){ echo $header_logo; }?>
			<div class="header-menu-content">
				<div class="container<?php if( $cookie_options['fullwidth-header-menu'] == '1'){ echo '-fluid'; } ?>">
					...
					<div class="header-menu clearfix">
						<div class="tab-header-menu-toggle header-menu-toggle toggle-nav-menu <?php if( !empty($cookie_options['header-menu-link-color-2']) ){ echo 'toggle-nav-menu-additional '; }?>">
							<div><a href="#"><div class="burg"></div></a></div>
						</div> 
						<?php //if( $cookie_options['header-menu-style'] != 'side-header-menu' ){ echo $header_menu_icons; } ?>
						<?php echo $header_menu_icons;  ?>
						<?php if( $cookie_options['header-menu-style'] == 'minimal' ){ ?>
							<div class="header-menu-toggle toggle-nav-menu <?php if( !empty($cookie_options['header-menu-link-color-2']) ){ echo 'toggle-nav-menu-additional '; }?>"><?php if( !empty($cookie_options['header-menu-name']) ){ ?><div><?php echo esc_attr($cookie_options['header-menu-name']); ?></div><?php } ?><div><a href="#"><div class="burg"></div></a></div>
							</div> 
							<?php }
							else{?>
							<nav class="nav-menu <?php if( !empty($cookie_options['header-menu-link-color-2']) ){ echo 'nav-menu-additional-color '; }?>page-scroll" >
								<?php /* Main Nav menu */ 
									echo $header_menu;
								?> 
							</nav>	
			...
			<nav class="tab-nav-menu page-scroll" >
				<?php /* Mobile Nav menu */ 
				wp_nav_menu(array( 'menu_class' => 'tab-nav-menu-content container-fluid', 'menu_id' => 'tab-navigation', 'container' => false, 'theme_location' => 'primary', 'fallback_cb'     => 'wp_page_menu' ) ); ?>
			</nav>
			<?php if( $cookie_options['header-menu-style'] == 'side-header-menu' ){ echo $header_menu_icons; } ?>
			<?php if( $cookie_options['header-menu-style'] == 'centered-menu' ){ echo $header_logo; }?>
			<?php if( $cookie_options['header-menu-style'] == 'side-header-menu' ){ ?>
				<div class="site-info">
					<?php if(!empty($cookie_options['footer-logo']['url'])){  ?>
						<div class="footer-logo"><a href="<?php echo esc_url( home_url( '/' ) ); ?>" ><img src="<?php echo esc_url($cookie_options['footer-logo']['url']); ?>" alt="<?php echo esc_attr( get_bloginfo( 'name' ) ); ?>" /></a></div>
					<?php }  ?>
	...
	<div id="content" class="site-content content <?php if( $cookie_options['header-menu-style'] == 'side-header-menu' ){ echo 'side-header-menu-content'; } ?>">
<?php } 
